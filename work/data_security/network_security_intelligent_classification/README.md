...
| `b01_target_aug_high.csv` | `0.70584` | 当前已知最高，目标域增强方向有效 |
...
1. `b01_target_aug_high.csv`
2. `b02_target_aug_mid.csv`

`b01_target_aug_high.csv` 是当前已知最高线上文件。它在 `m01` 基础上加入两个类别条件目标域增强模型，和 `m01` 相比改动 `516` 行，和 `d02` 相比改动 `594` 行。本地 OOF 为 `0.9328080314`，A 榜为 `0.70584`。

`b02_target_aug_mid.csv` 是较稳备选。它相对 `m01` 改动 `397` 行，风险低于 `b01`。
...
- 当前优先级：先提交 `upload_ready_breakthrough\b01_target_aug_high.csv`。
